...
from Attention.run_attention import find_tfl_lights


def load_binary_file(data_dir: str, crop_shape: tuple = (81, 81)) -> dict:
...
def viz_my_data(images: np.memmap, labels: np.memmap, predictions=None, num: tuple = (5, 5),
                labels2name={0: 'No TFL', 1: 'Yes TFL'}) -> None:
...
def load_tfl_data(url: str, suffix: str) -> list:
    ...
    for subdir, dirs, files in os.walk(url):
        for dir in dirs:
            data_list += glob.glob(os.path.join(url + '\\' + dir, suffix))
...
def load_data(dir_set: str) -> dict:
    ...
    url_image = "./leftImg8bit_trainvaltest/leftImg8bit/"
    url_label = "./gtFine/"
...
def get_rand_pixel(pixels: list) -> (int, int):
...
def crop_image(image: np.ndarray, x, y, size: int) -> np.ndarray:
    x += size
    y += size
    result_crop = image[y - size:y + size + 1, x - size:x + size + 1]

    return result_crop


def save_image(dir_name: str, image, label: int) -> None:
...
def call_write_to_bin_file(x_coords, y_coords, label: np.ndarray, image, dir_name: str) -> None:
...
def main():
    ...
    dataset = {'train': load_binary_file(train_path)}
