from Integration.tfl_manager import TFLManager


def load_pls_file(pls_path: str) -> list:
    with open(pls_path, 'r', encoding="utf8") as pls_file:
...
class Controller:
    def __init__(self, pls_path):
        ...
        self.frames_path = self.pls_data[1:]

    def run(self) -> None:
        for index, frame_path in enumerate(self.frames_path):
