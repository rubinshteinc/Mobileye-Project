...
from Detection.dataset_creation import crop_image, padding_image
from Attention.run_attention import find_tfl_lights
from Distance_Estimation.SFM import calc_TFL_dist
from Distance_Estimation.SFM_standAlone import FrameContainer
...
def visualize(prev_frame: Frame, curr_frame: Frame) -> None:
    ...
    if len(curr_frame.tfl_distance):
        for idx, point in enumerate(curr_frame.tfl_candidates):
            dis.text(point[0], point[1], r'{0:.1f}'.format(curr_frame.tfl_distance[idx, 2], color='y'))
...
class TFLManager:
    def __init__(self, pkl_path: str) -> None:
        self.net = tf.keras.models.load_model('../Detection/model10.h5')
