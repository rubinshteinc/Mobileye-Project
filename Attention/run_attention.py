...
def find_tfl_lights(c_image: np.ndarray, **kwargs):
    ...
    img = np.asarray(c_image)

    red = img[:, :, 0]
    green = img[:, :, 1]

    grad_r = sg.convolve2d(red, kernel_r, mode='same')
    grad_g = sg.convolve2d(green, kernel_g, mode='same')
    coordinates_red = peak_local_max(grad_r, min_distance=20, num_peaks=10)
    coordinates_green = peak_local_max(grad_g, min_distance=20, num_peaks=10)
